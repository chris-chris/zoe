...
program
  ...
  .action((srs_path: string, logrows: string, commitment?: string) => {
    gen_srs_cmd(srs_path, parseInt(logrows), commitment || DEFAULT_COMMITMENT);
  });
...
program
  ...
  .action(
    async (
      ...
      await get_srs_cmd(srs_path, settings_path, parseInt(logrows), commitment);
    }
  );
...
program
  ...
  .action((model: string, args: string[]) => {
    table(model || DEFAULT_MODEL, args);
  });
...
program
  .command(
    "calibrate-settings <model> <settings_path> <data> <target> <lookup_safety_margin> <scales> <scale_rebase_multiplier> [max_logrows] [only_range_check_rebase]"
  )
...
program
  .command(
    "gen-witness <data> <compiled_circuit> <output> <vk_path> <srs_path>"
  )
...
program
  ...
  .action((model: string, witness: string) => {
    mock(model || DEFAULT_MODEL, witness || DEFAULT_WITNESS);
  });
// Define the 'create-evm-verifier' command
program
  .command(
    "create-evm-verifier <srs_path> [vk_path] [settings_path] [sol_code_path] [abi_path] [render_vk_separately]"
  )
...
program
  .command(
    "create-evm-vk <srs_path> [vk_path] [settings_path] [sol_code_path] [abi_path]"
  )
...
program
  .command(
    "create-evm-data-attestation <witness> [settings_path] [sol_code_path] [abi_path] [data]"
  )
...
program
  .command(
    "create-evm-verifier-aggr <srs_path> <aggregation_settings> [vk_path] [sol_code_path] [abi_path] [logrows] [render_vk_separately]"
  )
...
program
  ...
  .action(
    (model: string, compiled_circuit?: string, settings_path?: string) => {
      compile_circuit(
        model || DEFAULT_MODEL,
        compiled_circuit || DEFAULT_COMPILED_CIRCUIT,
        settings_path || DEFAULT_SETTINGS
...
program
  .command(
    "setup <srs_path> <witness> [compiled_circuit] [vk_path] [pk_path] [disable_selector_compression]"
  )
