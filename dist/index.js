...
program.version("1.0.0").description("ZOE: ZKML Optimization Engine");
...
program
    ...
    .action(async (srs_path, settings_path, logrows, commitment) => {
    await (0, execute_1.get_srs_cmd)(srs_path, settings_path, parseInt(logrows), commitment);
});
...
program
    ...
    .action(async (model, settings_path, data, target, lookup_safety_margin, scales, scale_rebase_multiplier, max_logrows, only_range_check_rebase) => {
    const result = await (0, execute_1.calibrate)(model || default_1.DEFAULT_MODEL, data || default_1.DEFAULT_DATA, settings_path || default_1.DEFAULT_SETTINGS, target, lookup_safety_margin, scales, scale_rebase_multiplier, only_range_check_rebase ?? default_1.DEFAULT_ONLY_RANGE_CHECK_REBASE, max_logrows);
...
program
    ...
    .action(async (data, compiled_circuit, output, vk_path, srs_path) => {
    const result = await (0, execute_1.gen_witness)(compiled_circuit || default_1.DEFAULT_COMPILED_CIRCUIT, data || default_1.DEFAULT_DATA, output || default_1.DEFAULT_WITNESS, vk_path, srs_path);
    console.log(JSON.stringify(result));
});
...
program
    ...
    .action(async (srs_path, vk_path, settings_path, sol_code_path, abi_path, render_vk_separately) => {
    await (0, execute_1.create_evm_verifier)(vk_path || default_1.DEFAULT_VK, srs_path, settings_path || default_1.DEFAULT_SETTINGS, sol_code_path || default_1.DEFAULT_SOL_CODE, abi_path || default_1.DEFAULT_VERIFIER_ABI, render_vk_separately ?? default_1.DEFAULT_RENDER_VK_SEPARATELY);
});
...
program
    ...
    .action(async (srs_path, vk_path, settings_path, sol_code_path, abi_path) => {
    await (0, execute_1.create_evm_vk)(vk_path || default_1.DEFAULT_VK, srs_path, settings_path || default_1.DEFAULT_SETTINGS, sol_code_path || default_1.DEFAULT_VK_SOL, abi_path || default_1.DEFAULT_VK_ABI);
});
...
program
    ...
    .action(async (witness, settings_path, sol_code_path, abi_path, data) => {
    await (0, execute_1.create_evm_data_attestation)(settings_path || default_1.DEFAULT_SETTINGS, sol_code_path || default_1.DEFAULT_SOL_CODE_DA, abi_path || default_1.DEFAULT_VERIFIER_DA_ABI, data || default_1.DEFAULT_DATA, witness);
});
...
program
    ...
    .action(async (srs_path, aggregation_settings, vk_path, sol_code_path, abi_path, logrows, render_vk_separately) => {
    await (0, execute_1.create_evm_aggregate_verifier)(vk_path || default_1.DEFAULT_VK, srs_path, sol_code_path || default_1.DEFAULT_SOL_CODE_AGGREGATED, abi_path || default_1.DEFAULT_VERIFIER_AGGREGATED_ABI, aggregation_settings, logrows ? parseInt(logrows) : default_1.DEFAULT_AGGREGATED_LOGROWS, render_vk_separately ?? default_1.DEFAULT_RENDER_VK_SEPARATELY);
});
...
program
    ...
    (0, execute_1.setup)(compiled_circuit || default_1.DEFAULT_COMPILED_CIRCUIT, srs_path, vk_path || default_1.DEFAULT_VK, pk_path || default_1.DEFAULT_PK, witness, disable_selector_compression ?? default_1.DEFAULT_DISABLE_SELECTOR_COMPRESSION);
});
